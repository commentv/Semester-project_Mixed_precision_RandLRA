...
%single precision nystrom epsilon pseudo inverse algorithm is applied on a 
%chosed test problem
...
p = 0.5; % rate of decay: 0.5 slow, 1 med, 2 fast
...
%Perform the approximation in single precision and when everything is in
%single precision
for j = 1:length(k_vec)
    k = k_vec(j);
    for m = 1:number_trials
        
        [U,lambda] = Nystrom_eps_pinv(A,n,k,l,'s',m);
...
title(ax_1,sprintf('%s, %s, eps pinv',example,param))
...
legend(ax_1,'SVD','single','all single','Location','northeast');